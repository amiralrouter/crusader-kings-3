...
namespace Crusader_Kings_3.Module {
    public partial class ModulePlayer : UserControl {
        ...
        public ModulePlayer() {
            ...
            TraitManager.GetList().ForEach(trait => {
                TraitListViewer.AddItem(trait.logo, trait.id);
            });
            ...
            Utils.CreateTimer(1000, () => {
                ...
                TraitListViewer.SetSelecteds(player.traits.list);
            });
        }
        ...
        private void TraitList_Change(object sender, int[] ids) {
            if (player != null)
                player.traits.list = ids;
        }
